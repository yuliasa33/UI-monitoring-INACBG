...
export class SetupDebiturService {
  ...
  onGetAll(): Observable<HttpResponseModel<DebiturModel[]>> {
    return this.httpOperationService.onGetRequest(GET_ALL_DEBITUR);
  }

  onGetById(DebiturId: number): Observable<HttpResponseModel<DebiturModel>> {
    return this.httpOperationService.onGetRequest(GET_SETUP_DEBITUR_BY_ID + DebiturId);
  }

  onPostSave(Data: DebiturInsert): Observable<HttpResponseModel> {
    return this.httpOperationService.onPostRequest(POST_SAVE_SETUP_DEBITUR, Data)
  }

  onPutEdit(Data: DebiturModel): Observable<HttpResponseModel> {
    return this.httpOperationService.onPutRequest(PUT_UPDATE_SETUP_DEBITUR, Data)
  }

  onDelete(DebiturId: number): Observable<HttpResponseModel> {
    return this.httpOperationService.onDeleteRequest(DELETE_SETUP_DEBITUR + DebiturId)
  }

  onGetAllByPersonId(PersonId: number): Observable<HttpResponseModel<DebiturModel[]>> {
    return this.httpOperationService.onGetRequest(GET_ALL_DEBITUR_FOR_LOOKUP_ADMISI + PersonId);
  }
}
